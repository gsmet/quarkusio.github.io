...
- It relies on a closed-world assumption and removes most runtime dynamism (at least in its current form - work on the GraalVM side could change this in the future). This was not an issue for Quarkus as it makes the same assumption.
...
Although reducing memory footprint is not yet a primary goal of Leyden, it may occur as a side effect.
For example, if JAR files no longer need to remain open for class loading, some memory may be saved.

That said, Project Leyden is not magic either. In its current form:

1- Startup and warmup time improvements are significant, but not as substantial as with GraalVM/Mandrel native image.
...
With the advent of our AOT support, Quarkus now provides three modes of execution of the target artifact: plain JVM, AOT with Leyden and Native. Which one is best, depends on the use case. As always, weighing the pros and cons of each is necessary.
