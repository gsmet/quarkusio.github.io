...
In a previous https://quarkus.io/blog/leyden-1/[blog post], we shared our excitement about Project Leyden and how it gave us a new perspective on Java application startup performance.
...
- It relies on a closed-world assumption and removes most runtime dynamism (at least in its current form - work on the GraalVM side could change this in the future). Note: This was and is not an issue for Quarkus as it makes the same assumption; it can be problematic for third-party libraries.
...
- During a training phase, it records application behavior and gathers information such as loaded and linked classes, and in more recent versions also starts recording some JIT output - increasing scope and optimisation potentials in each JVM version.
...
For all the reasons <<graalvm,we've outlined>>, use native when it makes sense. Also because throughput will suffer.
...
That said, in the current state of things, native executables still start faster and remain unmatched in certain areas,
such as container image size and memory footprint.
Depending on your use case, for example, in serverless environments, they may be a better fit.
...
Project Leyden is being continuously improved and is likely to figure prominently in the future of the Java platform; as part of our quest in Quarkus to provide the best
development platform, with the help of the OpenJDK team at IBM, we are actively monitoring the progress made upstream (in the `premain` branch of https://github.com/openjdk/leyden/tree/premain[`git@github.com:openjdk/leyden.git`])
and plan to continue shaping the Quarkus + Leyden experience accordingly.

With the advent of our AOT support, Quarkus now provides three modes of execution of the target artifact: plain JVM, AOT with Leyden and Native.
Which one is best, depends on the use case.
As always, weighing the pros and cons of each is necessary.
